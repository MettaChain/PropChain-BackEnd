...
import { of } from 'rxjs';
import { ApiVersionEnum, SUPPORTED_API_VERSIONS, DEFAULT_API_VERSION } from './api-version.constants';
...
describe('VersionHeaderInterceptor', () => {
  ...
  describe('version validation', () => {
    it('should throw BadRequestException for unsupported versions', () => {
      ...
      expect(() => {
        interceptor.intercept(mockExecutionContext as ExecutionContext, mockCallHandler as CallHandler).subscribe();
      }).toThrow(`API version "v3" is not supported. Supported versions: ${SUPPORTED_API_VERSIONS.join(', ')}`);
    });
    ...
    it('should add deprecation headers for deprecated versions', (done) => {
      ...
      interceptor.intercept(mockExecutionContext as ExecutionContext, mockCallHandler as CallHandler).subscribe({
        next: () => {
          ...
          expect(mockSetHeader).toHaveBeenCalledWith('Warning', expect.stringContaining('API version v1 is deprecated'));
          expect(mockSetHeader).toHaveBeenCalledWith('X-API-Deprecation-Date', expect.any(String));
          done();
        },
        error: done
      });
