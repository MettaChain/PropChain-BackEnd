...
describe('DocumentUploadService', () => {
  ...
  describe('validate', () => {
    it('should pass for valid request', () => {
      const req: UploadRequest = { fileName: 'test.pdf', mimeType: 'application/pdf', fileSizeBytes: 1024 };
      expect(() => service.validate(req)).not.toThrow();
    });

    it('should throw BadRequestException for unsupported mime type', () => {
      const req: UploadRequest = { fileName: 'test.txt', mimeType: 'text/plain', fileSizeBytes: 1024 };
    ...
    it('should throw BadRequestException for file exceeding size limit', () => {
      const req: UploadRequest = { fileName: 'large.pdf', mimeType: 'application/pdf', fileSizeBytes: 11 * 1024 * 1024 };
      expect(() => service.validate(req)).toThrow(BadRequestException);
    ...
    it('should throw BadRequestException for empty file name', () => {
      const req: UploadRequest = { fileName: '   ', mimeType: 'application/pdf', fileSizeBytes: 1024 };
      expect(() => service.validate(req)).toThrow(BadRequestException);
  ...
  describe('prepareMetadata', () => {
    it('should sanitize filename and add uploadedAt', () => {
      const req: UploadRequest = { fileName: 'My File Report!.pdf', mimeType: 'application/pdf', fileSizeBytes: 1024 };
